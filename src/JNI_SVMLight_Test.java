...
public class JNI_SVMLight_Test {
  ...
  public static int N = 1000; // number of training docs

  public static int M = 2; // max. number of features per doc
  ...
  public static void main(String[] args) throws Exception {
    ...
    for (int i = 0; i < N; i++) {
      ...
      double[] values = new double[nDims];
        ...
        for (int j = 0; j < M; j++) {
          dims[j] = j + 1;
            //double tm = rd.nextDouble() * (i < N / 2 ? -1 : +1);
            double tm = (rd.nextDouble() * 0.5 + (i < N / 2 ? 0.5 : 0))*(rd.nextDouble() < 0.5 ? -1 : 1);
            //tm = tm+2;
    ...
    //SVMLightModel model = trainer.trainModel(traindata, tp);
      ...
      SVMLightModel  model = trainer.trainModel(traindata, new String[] { "-t", "1", "-d", "2" });
      model.writeModelToFile("model.file");
    ...
    System.out.println("\nVALIDATING SVM-light MODEL in Java..");
      ...
      for (int i = 0; i < N; i++) {
          ...
          for (int j = 0; j < M; j++) {
              dims[j] = j + 1;
              double tm = rd.nextDouble() * (i < N / 2 ? -1 : +1) * (j % 10 < 4 ? -1 : +1);
